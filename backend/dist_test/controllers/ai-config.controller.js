...
async function ensureSettingsTable() {
    const exists = await database_1.db.schema.hasTable('bot_settings');
    if (!exists) {
        await database_1.db.schema.createTable('bot_settings', (table) => {
...
async function getAIConfig(_req, res) {
    try {
        await ensureSettingsTable();
        const rows = await (0, database_1.db)('bot_settings').select('key', 'value');
        const settings = { ...DEFAULT_SETTINGS };
        for (const row of rows) {
            settings[row.key] = row.value;
        }
        // Also fetch learning stats for the dashboard
        const memoryCount = await (0, database_1.db)('bot_memory')
            .where('is_active', true)
            .count('id as count')
            .first();
        const totalLeads = await (0, database_1.db)('leads')
            .count('id as count')
            .first();
        const botActiveLeads = await (0, database_1.db)('leads')
        ...
        res.json({
            ...
            data: {
                settings,
                stats: {
                    activeMemoryPatterns: parseInt(String(memoryCount?.count || '0'), 10),
                    totalLeads: parseInt(String(totalLeads?.count || '0'), 10),
                    botActiveLeads: parseInt(String(botActiveLeads?.count || '0'), 10),
                },
            },
        });
    }
...
async function updateAIConfig(req, res) {
    try {
        ...
        for (const [key, value] of validUpdates) {
            const existing = await (0, database_1.db)('bot_settings').where('key', key).first();
            if (existing) {
                await (0, database_1.db)('bot_settings').where('key', key).update({
                    value: String(value),
                    updated_at: new Date(),
                });
            }
            else {
                await (0, database_1.db)('bot_settings').insert({
                    key,
                    value: String(value),
                    updated_at: new Date(),
                });
