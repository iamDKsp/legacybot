...
exports.runAutoMigrations = runAutoMigrations;
...
async function runAutoMigrations() {
    ...
    try {
        ...
        `);
        await database_1.db.raw(`CREATE INDEX IF NOT EXISTS idx_memory_category ON bot_memory(category)`);
        await database_1.db.raw(`CREATE INDEX IF NOT EXISTS idx_memory_area      ON bot_memory(legal_area)`);
        await database_1.db.raw(`CREATE INDEX IF NOT EXISTS idx_memory_usage     ON bot_memory(usage_count DESC)`);
        await database_1.db.raw(`CREATE INDEX IF NOT EXISTS idx_memory_active    ON bot_memory(is_active)`);
        // ── 6. bot_handoffs ──────────────────────────────────────────────────
        await database_1.db.raw(`
        ...
        `);
        await database_1.db.raw(`CREATE INDEX IF NOT EXISTS idx_handoff_lead ON bot_handoffs(lead_id)`);
        await database_1.db.raw(`CREATE INDEX IF NOT EXISTS idx_handoff_ack  ON bot_handoffs(acknowledged_at)`);
        // ── 7. bot_prompts ───────────────────────────────────────────────────
        await database_1.db.raw(`
        ...
        `);
        await database_1.db.raw(`CREATE INDEX IF NOT EXISTS idx_prompts_funnel ON bot_prompts(funnel_slug)`);
        ...
        `);
        await database_1.db.raw(`CREATE INDEX IF NOT EXISTS idx_knowledge_funnel ON knowledge_files(funnel_slug)`);
        // Colunas adicionais caso tabela já existia sem elas
        await database_1.db.raw(`ALTER TABLE knowledge_files ADD COLUMN IF NOT EXISTS extracted_text TEXT   DEFAULT NULL`).catch(() => { });
        await database_1.db.raw(`ALTER TABLE knowledge_files ADD COLUMN IF NOT EXISTS file_type      VARCHAR(50) DEFAULT NULL`).catch(() => { });
        await database_1.db.raw(`ALTER TABLE knowledge_files ADD COLUMN IF NOT EXISTS file_data      BYTEA  DEFAULT NULL`).catch(() => { });
        // ── 9. funnel_stages ─────────────────────────────────────────────────
        await database_1.db.raw(`
        ...
        const procStage = await (0, database_1.db)('stages').where({ slug: 'procuracao' }).first();
        const hasProcInFunnel = procStage
            ? await (0, database_1.db)('funnel_stages').where({ stage_id: procStage.id }).first()
            : null;
        const fsCount = await (0, database_1.db)('funnel_stages').count('id as c').first();
        const funelStageCount = parseInt(String(fsCount.c || '0'), 10);
        if (funelStageCount === 0 || hasProcInFunnel) {
            if (hasProcInFunnel) {
                console.log('[DB] 🔄 Detectado stage obsoleto (procuracao) — reconstruindo funnel_stages...');
                await (0, database_1.db)('funnel_stages').del();
            }
            const funnelDefs = {
                geral: [
            ...
            for (const [funnelSlug, stageDefs] of Object.entries(funnelDefs)) {
                const funnel = await (0, database_1.db)('funnels').where({ slug: funnelSlug }).first();
                if (!funnel)
                    continue;
                for (const def of stageDefs) {
                    const stage = await (0, database_1.db)('stages').where({ slug: def.slug }).first();
                    if (!stage)
                        continue;
                    await (0, database_1.db)('funnel_stages')
                        .insert({
                        funnel_id: funnel.id,
                        stage_id: stage.id,
            ...
            const jpExists = await (0, database_1.db)('phc_lawyers').where({ oab: 'OAB/SP nº 243.936' }).first();
            if (!jpExists) {
                console.log('[DB] 👨‍⚖️ Semeando Advogado Padrão (João Paulo Gabriel)...');
                await (0, database_1.db)('phc_lawyers').insert({
        ...
        const geralFunnel = await (0, database_1.db)('funnels').where({ slug: 'geral' }).first();
        if (geralFunnel) {
            const gCount = await (0, database_1.db)('funnel_stages').where({ funnel_id: geralFunnel.id }).count('id as c').first();
            const geralCount = parseInt(String(gCount.c || '0'), 10);
            if (geralCount === 0) {
        ...
        const count = await (0, database_1.db)('bot_memory').count('id as c').first();
        const memoryCount = parseInt(String(count.c || '0'), 10);
        if (memoryCount === 0) {
        ...
        await database_1.db.raw(`ALTER TABLE phc_lawyers ADD COLUMN IF NOT EXISTS cpf             VARCHAR(20)  DEFAULT NULL`).catch(() => { });
        await database_1.db.raw(`ALTER TABLE phc_lawyers ADD COLUMN IF NOT EXISTS email           VARCHAR(150) DEFAULT NULL`).catch(() => { });
        await database_1.db.raw(`ALTER TABLE phc_lawyers ADD COLUMN IF NOT EXISTS phone           VARCHAR(30)  DEFAULT NULL`).catch(() => { });
        await database_1.db.raw(`ALTER TABLE phc_lawyers ADD COLUMN IF NOT EXISTS address         VARCHAR(255) DEFAULT NULL`).catch(() => { });
        await database_1.db.raw(`ALTER TABLE phc_lawyers ADD COLUMN IF NOT EXISTS city            VARCHAR(100) DEFAULT NULL`).catch(() => { });
        await database_1.db.raw(`ALTER TABLE phc_lawyers ADD COLUMN IF NOT EXISTS state           CHAR(2)      DEFAULT NULL`).catch(() => { });
        await database_1.db.raw(`ALTER TABLE phc_lawyers ADD COLUMN IF NOT EXISTS additional_info TEXT         DEFAULT NULL`).catch(() => { });
        // Endereço dividido (v2)
        await database_1.db.raw(`ALTER TABLE phc_lawyers ADD COLUMN IF NOT EXISTS cep             VARCHAR(9)   DEFAULT NULL`).catch(() => { });
        await database_1.db.raw(`ALTER TABLE phc_lawyers ADD COLUMN IF NOT EXISTS street          VARCHAR(255) DEFAULT NULL`).catch(() => { });
        await database_1.db.raw(`ALTER TABLE phc_lawyers ADD COLUMN IF NOT EXISTS street_number   VARCHAR(20)  DEFAULT NULL`).catch(() => { });
        await database_1.db.raw(`ALTER TABLE phc_lawyers ADD COLUMN IF NOT EXISTS neighborhood    VARCHAR(100) DEFAULT NULL`).catch(() => { });
        await database_1.db.raw(`ALTER TABLE phc_lawyers ADD COLUMN IF NOT EXISTS complement      VARCHAR(100) DEFAULT NULL`).catch(() => { });
        await database_1.db.raw(`CREATE INDEX IF NOT EXISTS idx_lawyers_name ON phc_lawyers(name)`).catch(() => { });
        ...
        `).catch(() => { });
        await database_1.db.raw(`CREATE INDEX IF NOT EXISTS idx_phc_lead   ON phc_documents(lead_id)`).catch(() => { });
        await database_1.db.raw(`CREATE INDEX IF NOT EXISTS idx_phc_lawyer ON phc_documents(lawyer_id)`).catch(() => { });
        await ensureUpdatedAtTrigger('phc_documents').catch(() => { });
        // Colunas que podem faltar em tabelas antigas (idempotente)
        await database_1.db.raw(`ALTER TABLE phc_documents ADD COLUMN IF NOT EXISTS funnel_slug VARCHAR(100) DEFAULT NULL`).catch(() => { });
        await database_1.db.raw(`ALTER TABLE phc_documents ADD COLUMN IF NOT EXISTS notes       TEXT         DEFAULT NULL`).catch(() => { });
        await database_1.db.raw(`ALTER TABLE phc_documents ADD COLUMN IF NOT EXISTS file_path   VARCHAR(500) DEFAULT NULL`).catch(() => { });
        await database_1.db.raw(`ALTER TABLE phc_documents ADD COLUMN IF NOT EXISTS status      VARCHAR(20)  DEFAULT 'rascunho'`).catch(() => { });
        // Recriar constraint de status para garantir que inclui 'baixado'
        await database_1.db.raw(`ALTER TABLE phc_documents DROP CONSTRAINT IF EXISTS phc_documents_status_check`).catch(() => { });
        await database_1.db.raw(`ALTER TABLE phc_documents ADD CONSTRAINT  phc_documents_status_check CHECK (status IN ('rascunho','salvo','baixado'))`).catch(() => { });
        await database_1.db.raw(`CREATE INDEX IF NOT EXISTS idx_phc_funnel ON phc_documents(funnel_slug)`).catch(() => { });
        // ── DOC-1. documents: file_path + file_url ───────────────────────────
        await database_1.db.raw(`ALTER TABLE documents ADD COLUMN IF NOT EXISTS file_url  VARCHAR(500) DEFAULT NULL`).catch(() => { });
        await database_1.db.raw(`ALTER TABLE documents ADD COLUMN IF NOT EXISTS file_path VARCHAR(500) DEFAULT NULL`).catch(() => { });
