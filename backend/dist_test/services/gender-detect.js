...
function guessByEnding(n) {
    if (/A$/.test(n))
        return 'F';
    if (/(INA|ENA|ELA|ILA|OLA|ULA|ELLE|ETTE|ISSA)$/.test(n))
        return 'F';
    if (/(ON|OR|OS|IM|EM|SON|TON|ANDO|ALDO)$/.test(n))
        return 'M';
    return 'M';
}
function detectGender(fullName) {
    ...
    if (FEMALE.has(parts[0]))
        return 'F';
    if (MALE.has(parts[0]))
        return 'M';
    if (parts[1] && FEMALE.has(parts[1]))
        return 'F';
    if (parts[1] && MALE.has(parts[1]))
        return 'M';
    return guessByEnding(parts[0]);
}
const M_MAP = { solteiro: 'solteiro', casado: 'casado', divorciado: 'divorciado', viuvo: 'vi\u00FAvo' };
const F_MAP = { solteiro: 'solteira', casado: 'casada', divorciado: 'divorciada', viuvo: 'vi\u00FAva' };
function buildCtx(name, maritalStatus, occupation, override) {
...
//# sourceMappingURL=gender-detect.js.map