...
exports.detectLegalArea = detectLegalArea;
exports.detectObjection = detectObjection;
exports.detectPaymentQuestion = detectPaymentQuestion;
exports.detectTimelineQuestion = detectTimelineQuestion;
exports.detectEmotionalState = detectEmotionalState;
exports.extractCPF = extractCPF;
exports.extractName = extractName;
exports.runPostConversationLearning = runPostConversationLearning;
exports.cleanupStaleMemories = cleanupStaleMemories;
...
// ─────────────────────────────────────────────────────────────
function detectPaymentQuestion(text) {
    const lower = text.toLowerCase();
...
// ─────────────────────────────────────────────────────────────
function detectTimelineQuestion(text) {
    const lower = text.toLowerCase();
...
function detectEmotionalState(text) {
    const lower = text.toLowerCase();
    if (/nervoso|desesperado|urgente|preciso urgente|estou mal|socorro|desespero|não sei o que fazer|por favor me ajud/.test(lower))
        return 'anxious';
    if (/absurdo|revoltante|indignado|não aguento|ódio|palhaçada|ridículo|raiva|lixo|processem|vergonha/.test(lower))
        return 'angry';
    if (/obrigado|grato|que bom|aliviado|fico feliz|maravilha|muito bom|perfeito|excelente|top/.test(lower))
        return 'hopeful';
    return 'neutral';
}
// ─────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────
async function runPostConversationLearning(leadId) {
        ...
        for (const p of patterns) {
            ...
            if (existing) {
                await (0, database_1.db)('bot_memory')
                    ...
                    .update({
                    confidence_score: Math.min(100, existing.confidence_score + (p.converted ? 5 : 1)),
                    lead_converted: p.converted ? true : Boolean(existing.lead_converted),
                });
            }
            else {
...
async function cleanupStaleMemories() {
    try {
        ...
        const count = await (0, database_1.db)('bot_memory')
            .where('usage_count', '<', 2)
...
// ─────────────────────────────────────────────────────────────
async function getTopPatterns(category, limit = 20) {
...
async function getLearningSummary() {
    const [total, active] = await Promise.all([
        (0, database_1.db)('bot_memory').count('id as count').first(),
        (0, database_1.db)('bot_memory').where('is_active', true).count('id as count').first(),
    ]);
    const byCategory = await (0, database_1.db)('bot_memory')
    ...
    const recent = await (0, database_1.db)('bot_memory')
    ...
    return {
        totalPatterns: parseInt(String(total.count || '0'), 10),
        activePatterns: parseInt(String(active.count || '0'), 10),
        topCategories,
        recentlyLearned: parseInt(String(recent?.count || '0'), 10),
    };
}
