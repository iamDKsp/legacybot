...
exports.generateDocumentText = generateDocumentText;
...
function getAcaoByFunnel(slug) {
    const s = (slug || '').toLowerCase().replace(/[_\s]+/g, '-');
    if (s.includes('trabalhista'))
        return 'Reclamacao Trabalhista c/c Verbas Rescisorias';
...
function todayFormatted(city, state) {
...
function buildPrompt(docType, lead, lawyer) {
...
// ─── Funcao principal ─────────────────────────────────────────
async function generateDocumentText(docType, lead, lawyer) {
...
//# sourceMappingURL=phc-gemini.service.js.map