...
exports.getAcaoContrato = getAcaoContrato;
exports.getAcaoProcuracao = getAcaoProcuracao;
exports.advQual = advQual;
exports.clienteQual = clienteQual;
exports.buildContrato = buildContrato;
exports.buildProcuracao = buildProcuracao;
exports.buildDeclaracaoHipo = buildDeclaracaoHipo;
function norm(s) { return s.toLowerCase().replace(/[\s_]+/g, '-'); }
// -- Objetos por funil ------------------------------------------
function getAcaoContrato(slug) {
    const s = norm(slug);
    if (s.includes('trabalhista'))
        return 'RECLAMAÇÃO TRABALHISTA C/C VERBAS RESCISÓRIAS';
    if (s.includes('negativado') || s.includes('negativada'))
        return 'AÇÃO DE DANOS MORAIS C.C. INEXISTÊNCIA DE DÉBITOS COM PEDIDO DE TUTELA DE URGÊNCIA';
...
function getAcaoProcuracao(slug) {
    const s = norm(slug);
    if (s.includes('trabalhista'))
        return 'Reclamação Trabalhista c/c Verbas Rescisórias';
    if (s.includes('negativado') || s.includes('negativada'))
...
function clienteQual(g, name, rg, cpf, addr, city, state, cep) {
...
//# sourceMappingURL=phc-templates.js.map