...
export function advQual(adv:{name:string;oab:string;city?:string|null;state?:string|null;street?:string|null;street_number?:string|null;cep?:string|null}):string {
  ...
  const cep = adv.cep ? , CEP:  : '';
  return ${adv.name}, advogado inscrito na , com escritório profissional localizado à , na cidade de ;
}
...
export function clienteQual(g:GenderCtx, name:string, rg?:string|null, cpf?:string|null, addr?:string|null, city?:string|null, state?:string|null, cep?:string|null):string {
  const rgStr  = rg  ? inscrit no RG nº  : '';
  const cpfStr = cpf ? CPF nº  : '';
  const id     = [rgStr, cpfStr].filter(Boolean).join(' e ');
  const end    = [addr, city && state ? ${city}/ : city||state].filter(Boolean).join(', ');
  const cepStr = cep ? , CEP  : '';
  return ${name}, ;
}
...
export function buildContrato(d:ContratoData): string[] {
  ...
  return [
    ...
    ${d.advQualificacao}, doravante unicamente denominado de CONTRATADO e de outro lado, , doravante denominad CONTRATANTE; os quais pactuam o presente contrato conforme as cláusulas abaixo:,
    1 - DO OBJETO,
    1.1. O presente instrumento tem por objeto propor , com atuação até eventual recurso perante o Tribunal de Justiça.,
    2 - DOS HONORÁRIOS E DO VENCIMENTO,
    2.1.  se obriga a pagar como honorários contratados em relação às ações descritas na cláusula 1.2 a quantia de 50% (cinquenta por cento), sobre o valor total da condenação em fase de liquidação, bem como, sobre o valor de eventual multa cominatória. Ainda que exista acordo entre as partes, a porcentagem continua inalterada.,
    2.2. Ficará por conta  CONTRATANTE as despesas relativas às custas iniciais, preparo, taxa de procuração, honorários de sucumbência e demais taxas oriundas da referida ação;,
    2.3.  autoriza de livre e espontânea vontade que o CONTRATADO faça levantamento de qualquer depósito judicial nos autos, abatendo honorários contratuais e sucumbenciais, repassando o remanescente.,
    2.4.  fica ciente de que em caso de destituição ou revogação de poderes, deve pagar na integralidade os honorários contratados na cláusula 2.1, podendo o CONTRATADO, a seu critério, compensar tal percentual com eventual indenização concedida no processo.,
    ...
    3.1. Em caso de violação contratual, destituição do defensor ou inadimplemento,  pagará multa de 30% (trinta por cento) sobre o valor atualizado da causa, sem prejuízo de eventual ação por perdas e danos.,
    ...
    3.3.  obriga-se a atualizar seu endereço e números de telefone junto ao CONTRATADO, sempre que houver alteração.,
    4 - DAS CONSIDERAÇÕES FINAIS,
    4.1. Fica eleito o Foro da Comarca de , para apreciar todas as questões decorrentes do presente contrato.,
...
export function buildProcuracao(d:ProcuracaoData): string[] {
  ...
  return [
    ...
    ${d.clienteQualificacao}.\n\nConstitui seu bastante procurador , advogado inscrito na , com escritório profissional à , a quem confere os mais amplos e gerais poderes das cláusulas ad juditia et extra, para o foro em geral, não importando qual o Juízo, Instância ou Tribunal, (inclusive, instância administrativa ou fiscal), podendo propor contra qualquer pessoa física ou jurídica de direito público ou privado as ações que julgar necessárias à defesa de seus direitos, e defendê-l nas que contra el for proposta, seguindo-as até final liquidação, podendo, ainda e da mesma forma, intervir em qualquer caso judicial ou extra judicial de interesse  outorgante, fazer chamamento à autoria, abrir inventários segui-los até final partilha, requerer abertura de sindicâncias administrativas e defender  outorgante, nas que contra el forem abertas, fazer a defesa  outorgante em processo e inquéritos criminais, transigir, desistir, receber e dar quitações de qualquer espécie, pagar, assinar termos de levantamento e depósito, firmar compromissos, fazer acordos, concordar e discordar de quaisquer declarações, representá-l perante repartições públicas, autarquias ou de economia mista e substabelecer esta no todo ou em parte, com ou sem reservas de poderes, principalmente para representá-l na .  outorgante, ainda, confere ao seu procurador o direito de preferência para aquisição de eventual crédito judicial oriundo de ação por este proposta, obrigando-se a notificá-lo previamente sobre qualquer proposta de cessão, em igualdade de condições com terceiros, no prazo de 5 (cinco) dias úteis para manifestação.,
    d.localData,
  ];
}
